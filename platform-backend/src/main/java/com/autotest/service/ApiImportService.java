...
/**
 * 服务：接口导入
 * 职责：支持从 Postman 与 Swagger(OpenAPI3) 文件解析接口，验证格式后落库。
 * 说明：统一抽取 header/query/rest/body 等参数结构，生成内部 Api 实体。
 */
@Service
public class ApiImportService {
     ...
     * @param sourceType // 源类型（postman/swagger）
    ...
    /**
     * 递归收集 Postman item 下所有 API 信息
     *
     * @param apiInfoList // 聚合API信息的链表
     * @param apiList     // Postman item 列表（可能嵌套）
     * @return void       // 无返回
     */
    ...
    /**
     * 解析并保存 Swagger(OpenAPI3) 接口
     *
     * @param data      // Swagger JSON 内容
     * @param projectId // 项目ID
     * @param moduleId  // 模块ID
     * @param userId    // 操作用户ID
     * @return void     // 无返回
