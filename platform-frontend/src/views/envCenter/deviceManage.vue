 ...
 */
<template>
    <div class="device-list"> <!-- 设备管理页面容器：自适应宽度 -->
        <!-- 搜索 -->
        <el-form :inline="true" :model="searchForm"> <!-- 行内表单：关键词与状态过滤 -->
            <el-form-item label="">
                <el-input size="small" v-model="searchForm.condition" prefix-icon="el-icon-search" placeholder="请输入设备名称"/>
            </el-form-item>
            <el-form-item label="">
                <el-select size="small" clearable style="width:150px" v-model="searchForm.status" placeholder="请选择状态"> <!-- 状态筛选项 -->
        ...
        <!-- 筛选条件 -->
        <el-table class="filter-table" stripe :row-style="{height:35+'px'}" :data="filterData"> <!-- 条件勾选区域 -->
        ...
        <!-- 设备列表 -->
        <div class="device-table"> <!-- 设备卡片区域：按行分组展示 -->
                    ...
                    <div class="device-box-border">
                        <div class="device-box-info">
                            <div class="box-header">
                                ...
                                <el-button style="float: right" size="mini" v-if="device.status==='online'" type="primary" @click="useDevice(device)">立即使用</el-button> <!-- 空闲设备操作 -->
        ...
        <el-dialog title="编辑设备名称" :visible.sync="deviceVisible" width="600px" :append-to-body="true" destroy-on-close @closed="deviceVisible=false"> <!-- 编辑设备弹窗 -->
