...
export default {
    ...
    methods: {
        ...
        editCaseApi(index){
            ...
            this.isAddApi = false;	// 更新编辑页面的标识，用于区分add & edit
            this.activeTab = "body";	// 激活选项卡

						// 编辑新增接口（根据Body是否为空判断，通过则Post查询并赋值）
            if(!caseApi.body){
                ...
                this.$get(url, response =>{
                    ...
                    caseApi.settings = [];
                    caseApi.logics = [];
                    this.caseApiForm = caseApi;		// 编辑数据初赋值
                    this.editCaseApiVisible = true;
                });
            }

						// 编辑已有接口（body非空说明已存在DB）
						else{
								// JSON解析controller，并初赋值给绑定的响应数据
