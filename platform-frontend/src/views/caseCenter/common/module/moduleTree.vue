    ...
    <div class="module-tree">
        ...
        <div class="tree-input">
            <el-input size="mini" placeholder="输入模块名称过滤" v-model="filterText"/>
				...
				<!--tree-body-->
        <div class="tree-body">
            <el-tree class="filter-tree" :accordion="false" :data="treeData" :current-node-key="currentModule"
    ...
    dragNode(dragNode, tarNode, position, event) {
        ...
        if(position === "inner"){
            ...
            if(tarNode.data.id !== dragNode.data.parentId){
                this.$emit("dragNode", dragNode, tarNode.data.id);	//传参（拖拽结点，目标结点id）
            }
        }
				// 外拖拽
				else{
            ...
            if(tarNode.data.parentId !== dragNode.data.parentId ){
                this.$emit("dragNode", dragNode, tarNode.data.parentId);		// 传参（拖拽结点，目标结点parentID）
            }
        }
    },
