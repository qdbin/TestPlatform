...
export default {
    ...
    // 初始化：设置面包屑并加载项目列表与用户列表
    created() {
        this.$root.Bus.$emit('initBread', ["系统管理", "项目管理"]); // 更新面包屑导航
        this.getdata(this.searchForm); // 拉取项目列表
        this.getUsers(); // 拉取所有用户
    },
    // 业务方法
    methods: {
        // 列表查询与分页赋值
        ...
        submitProjectForm(confirm, form){
            this.$refs[confirm].validate(valid => {
                if (valid) {
                    ...
                    this.$post(url, form, response =>{ // 表单校验通过后提交新增
                        this.$message.success("保存成功"); // 成功提示
                        this.projectVisible = false; // 关闭弹窗
        ...
        recoverProject(projectId){
            ...
            .then(() => {
                ...
                this.$post(url, {id: projectId}, response => {
                    this.$message.success("恢复成功"); // 操作成功提示
                    this.getdata(this.searchForm); // 刷新列表
                });
            })
            .catch(() => {
