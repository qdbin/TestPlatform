...
class ConsoleLangChainCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_start(self, serialized, prompts, **kwargs):
        ...
        """
        app_logger.info(
            "llm_start model={} prompt_count={}", serialized.get("name"), len(prompts)
        )
        console.print(
            Fore.CYAN + f"[LLM START] prompts={len(prompts)}" + Style.RESET_ALL
        )
    ...
    def on_llm_end(self, response, **kwargs):
        ...
        console.print(
            Fore.GREEN + f"[LLM END] generations={len(generations)}" + Style.RESET_ALL
        )
    ...
    def on_tool_start(self, serialized, input_str, **kwargs):
        ...
        """
        app_logger.info(
            "tool_start name={} input={}", serialized.get("name"), input_str[:120]
        )
        console.print(
            Fore.YELLOW + f"[TOOL START] {serialized.get('name')}" + Style.RESET_ALL
        )
